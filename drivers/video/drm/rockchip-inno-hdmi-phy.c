                                     ...
                                     unsigned long rate)
{
        ...
        printf("inno_hdmi_phy_get_tmdsclk\n");
...
static int inno_hdmi_phy_power_off(struct rockchip_phy *phy)
{
        ...
        printf("inno_hdmi_phy_power_off\n");
...
static int inno_hdmi_phy_clk_is_prepared(struct inno_hdmi_phy *inno)
{
        ...
        printf("inno_hdmi_phy_clk_is_prepared\n");
...
{
        printf("inno_hdmi_phy_clk_prepare\n");
        if (inno->plat_data->dev_type == INNO_HDMI_PHY_RK3228)
                                      ...
                                      unsigned long rate)
{
        ...
        printf("inno_hdmi_phy_clk_set_rate\n");
...
{
	printf("inno_hdmi_phy_rk3328_init\n");
        /*
                              ...
                              const struct phy_config *phy_cfg)
{
	u32 val;
        ...
        printf("inno_hdmi_phy_rk3328_power_on\n");
        /* set pdata_en to 0 */
...
{
	printf("inno_hdmi_phy_rk3328_power_off\n");
        /* Power off driver */
...
static int inno_hdmi_phy_rk3328_pre_pll_update(struct inno_hdmi_phy *inno,
                                    const struct pre_pll_config *cfg)
{
	u32 val;

        printf("inno_hdmi_phy_rk3328_pre_pll_update\n");
        /* Power off PLL */
...
static unsigned long inno_hdmi_3328_phy_pll_recalc_rate(struct inno_hdmi_phy *inno,
                                   unsigned long parent_rate)
{
	unsigned long rate, vco, frac;
        ...
        printf("inno_hdmi_3328_phy_pll_recalc_rate\n");
...
static int inno_hdmi_update_phy_table(struct inno_hdmi_phy *inno, u32 *config,
                               struct phy_config *phy_cfg,
                               int phy_table_size)
{
        int i, j;

        printf("inno_hdmi_update_phy_table\n");
        for (i = 0; i < phy_table_size; i++) {
                phy_cfg[i].tmdsclock =
                        (unsigned long)config[i * 15];

                printf("%ld ", phy_cfg[i].tmdsclock);
                for (j = 0; j < 14; j++) {
                        phy_cfg[i].regs[j] = (u8)config[i * 15 + 1 + j];
                        printf("0x%02x ", phy_cfg[i].regs[j]);
                }
                printf("\n");
        }
...
static int inno_hdmi_phy_set_bus_width(struct rockchip_phy *phy, u32 bus_width)
{
        struct inno_hdmi_phy *inno = dev_get_priv(phy->dev);

	printf("in inno_hdmi_phy_set_bus_width");
        inno->bus_width = bus_width;

        return 0;
}

static long inno_hdmi_phy_clk_round_rate(struct rockchip_phy *phy, unsigned long rate)
{
	struct inno_hdmi_phy *inno = dev_get_priv(phy->dev);
        int i;
        const struct pre_pll_config *cfg = pre_pll_cfg_table;
        ...
        printf("inno_hdmi_phy_clk_round_rate\n");
        for (; cfg->pixclock != ~0UL; cfg++)
